export class HigLaboFlatpickr {
    ...
    public ReplaceMonthNav(instance: any) {
        ...
        instance.l10n.months.longhand.forEach((monthName: string, index: number) => {
        ...
        monthNav.appendChild(this.createFlatpickrMonthNavRow(
            "year",
            yearText,
    ...
    private createFlatpickrMonthNavRow(rowName: string, textElement: HTMLElement, previous: () => void, next: () => void) {
    ...
    private createFlatpickrMonthNavButton(direction: string, click: () => void) {
    ...
    private setFlatpickrYear(instance: any, yearText: HTMLInputElement) {
    ...
    private getInputDateText(instance: any) {
        ...
        if (input.getAttribute("input-date") == "true") {
            const textInput = input.querySelector("input[type='text']") as HTMLInputElement | null;
            if (textInput != null) {
