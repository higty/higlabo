...
namespace HigLabo.Net.Smtp
{
    public class ContentType
    {
        ...
        public static String CreateInitializeGeneratedFileExtensionsMethodBodyText()
        {
            var d = GetMimeContentTypesFromRegistry();
            StringBuilder sb = new StringBuilder();

            foreach (var key in d.Keys.OrderBy(el => el))
            {
                sb.AppendFormat("d[\"{0}\"] = \"{1}\";", key, d[key]);
                sb.AppendLine();
            }
            return sb.ToString();
        }
